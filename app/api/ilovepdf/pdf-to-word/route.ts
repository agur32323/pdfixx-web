...
export async function POST(req: Request) {
  try {
    ...
    // ✅ 1) start task (PDF -> Word için doğru tool: pdfoffice)
    const startRes = await fetch(`${ILOVE_BASE}/start/pdfoffice`, {
      method: "GET",
    ...
    return new NextResponse(bytes, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="pdf_to_word_${Date.now()}.docx"`,
        "Cache-Control": "no-store",
      },
    });
  } catch (e: any) {
    return NextResponse.json(
      { error: e?.message ?? "pdf-to-word error" },
      { status: 500 }
    );
  }
}